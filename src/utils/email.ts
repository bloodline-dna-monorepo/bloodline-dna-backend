...
`
  ...
  static async sendPasswordResetEmail(email: string, token: string, fullName?: string): Promise<boolean> {
    // Tạo URL với token được encode để tránh lỗi URL
    const resetUrl = `${process.env.CORS_ORIGIN || "http://localhost:3000"}/reset-password?token=${encodeURIComponent(token)}`
